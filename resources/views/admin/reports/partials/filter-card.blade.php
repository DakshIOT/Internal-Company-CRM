...
>
    <div class="flex flex-col gap-4 border-b border-slate-100 pb-4 md:flex-row md:items-start md:justify-between">
        <div>
            ...
            <h2 class="mt-2 font-display text-2xl font-semibold text-slate-950">Explicit admin report scope</h2>
            <div class="mt-3 flex flex-wrap gap-2">
                ...
                @endif
                @if ($filters->userId)
                    <span class="crm-chip bg-slate-100 text-slate-600">{{ $selectedEmployee?->name ?? 'Employee' }}</span>
                @endif
                @if ($filters->employeeRole)
                    <span class="crm-chip bg-slate-100 text-slate-600">{{ Role::label($filters->employeeRole) }}</span>
                @endif
            ...
            <div class="grid gap-4 lg:grid-cols-4">
                ...
                @if ($supportsVenue)
                    <label class="crm-field">
                        ...
                        <select name="venue_id" class="crm-input">
                ...
                <label class="crm-field">
                    <span class="crm-field-label">Employee Type</span>
                    <select name="employee_role" class="crm-input">
                        <option value="">All roles</option>
                        @foreach ($filterOptions['employee_roles'] as $optionValue => $optionLabel)
                            <option value="{{ $optionValue }}" @selected($filters->employeeRole === $optionValue)>{{ $optionLabel }}</option>
                        @endforeach
                    </select>
                </label>
                ...
                @if ($supportsVendor)
                    <label class="crm-field" x-show="moduleValue === '{{ ReportModule::VENDOR_ENTRIES }}' || moduleValue === ''" x-cloak>
                        ...
                        <select name="vendor_id" class="crm-input">
                ...
                @if ($supportsPackageService)
                    <label class="crm-field" x-show="moduleValue === '{{ ReportModule::FUNCTIONS }}' || moduleValue === ''" x-cloak>
                        ...
                        <select name="package_id" class="crm-input">
